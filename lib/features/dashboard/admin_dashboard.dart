...
class AdminDashboard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final servingTables = store.tables.where((t) => t.status == TableStatus.serving).length;
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text('Xin chào, ' + (auth.currentUser?.fullName.split(' ').last ?? '')),
        actions: [
          IconButton(
      ...
      body: RefreshIndicator(
        ...
        child: ListView(
          ...
          children: [
            ...
            GridView.count(
              ...
              children: [
                ...
                StatCard(
                  ...
                  value: servingTables.toString() + ' / ' + store.tables.length.toString(),
                  icon: Icons.table_restaurant,
                  color: AppColors.primary,
                ),
              ],
            ),
            if (isDown || lowStock.isNotEmpty || restock.isNotEmpty) ...[
              ...
              if (slowProducts.isNotEmpty)
                _alertCard(
                  ...
                  subtitle: 'Các món bán chậm: ' + slowProducts.take(3).map((e) => e.key.name).join(', '),
            ...
            const SizedBox(height: 20),
            _sectionTitle('🔥 Top 5 món bán chạy', onMore: () => context.go('/reports')),
            const SizedBox(height: 8),
            if (topProducts.isEmpty)
              const EmptyState(emoji: '📊', title: 'Chưa có dữ liệu')
            else
              ...topProducts.asMap().entries.map((e) => _topRow(e.key + 1, e.value.key.name, e.value.key.emoji, e.value.value)),
            const SizedBox(height: 20),
            _sectionTitle('🧾 Đơn hàng gần đây', onMore: () => context.go('/orders')),
            const SizedBox(height: 8),
            ...recentOrders.map((o) => Card(
                  child: ListTile(
                    onTap: () => context.go('/orders/' + o.id),
                    leading: const Icon(Icons.receipt, color: AppColors.primary),
                    title: Text(o.orderCode + ' • ' + (o.tableName ?? 'Mang đi')),
                    subtitle: Text(Fmt.relative(o.createdAt) + ' • ' + Fmt.money(o.total)),
  ...
  Widget _topRow(int rank, String name, String emoji, int count) => Card(
        child: ListTile(
          ...
          trailing: Text(count.toString() + ' ly', style: const TextStyle(fontWeight: FontWeight.w600)),
        ),
      );
  ...
  void _showNotifications(BuildContext context, DataStore store) {
    showModalBottomSheet(
      ...
      builder: (ctx) {
        ...
        if (list.isEmpty) {
          return const SizedBox(height: 200, child: EmptyState(emoji: '🔔', title: 'Không có thông báo'));
        }
        return ListView.builder(
          ...
          itemBuilder: (_, i) {
            ...
            return ListTile(
              ...
              trailing: Text(Fmt.relative(n.createdAt), style: const TextStyle(fontSize: 11)),
            );
          },
        );
      },

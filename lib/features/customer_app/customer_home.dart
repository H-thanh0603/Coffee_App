...
class _CustomerHomeState extends State<CustomerHome> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text('Chào ' + (auth.currentUser?.fullName.split(' ').last ?? '')),
        bottom: TabBar(
          controller: _tab,
        ...
        child: ListView(
          ...
          children: [
            Padding(padding: const EdgeInsets.symmetric(horizontal: 4),
              child: ChoiceChip(label: const Text('Tất cả'), selected: _catId == null,
...
class _MyOrdersTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final auth = context.watch<AuthProvider>();
    final store = context.watch<DataStore>();
    final myOrders = store.orders.where((o) =>
        o.customerId == auth.currentUser?.id || o.cashierId == auth.currentUser?.id).toList()
        ...
        return Card(
          child: ListTile(
            ...
            title: Text(o.orderCode + ' • ' + Fmt.money(o.total),
                style: const TextStyle(fontWeight: FontWeight.w600)),
            subtitle: Text(o.itemCount.toString() + ' món • ' + Fmt.relative(o.createdAt) +
                '\n' + o.orderStatus.label),
            isThreeLine: true,
          ),
        );
      ...
      itemBuilder: (_, i) {
        ...
        return Card(
          ...
          child: ListTile(
            ...
            title: Text(v.code, style: const TextStyle(fontWeight: FontWeight.w800)),

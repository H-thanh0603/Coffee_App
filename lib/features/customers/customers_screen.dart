...
class _CustomersScreenState extends State<CustomersScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    var list = store.customers.where((c) =>
      _q.isEmpty || c.fullName.toLowerCase().contains(_q.toLowerCase()) || c.phone.contains(_q)).toList();
    ...
    return Scaffold(
      ...
      body: Column(children: [
        Padding(
          ...
          child: TextField(
            decoration: const InputDecoration(hintText: 'Tìm theo tên/SĐT', prefixIcon: Icon(Icons.search), isDense: true),
                    ...
                    return Card(
                      child: ListTile(
                        leading: CircleAvatar(
                          ...
                          child: Text(c.fullName.characters.first, style: const TextStyle(color: Colors.white)),
                        ),
                        title: Row(children: [
                          Expanded(child: Text(c.fullName, style: const TextStyle(fontWeight: FontWeight.w600))),
                          Container(
                            ...
                            child: Text(c.rank.label, style: TextStyle(fontSize: 11, color: Color(c.rank.colorValue), fontWeight: FontWeight.w600)),
                          ),
                        ]),
                        subtitle: Text(c.phone + ' • ' + c.points.toString() + ' điểm • ' + Fmt.money(c.totalSpent)),
                      ),
                    );
                  },
                ),
